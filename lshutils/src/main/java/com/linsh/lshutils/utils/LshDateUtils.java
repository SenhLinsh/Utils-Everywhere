...
public class LshDateUtils {
     ...
     *
     * @param time  时间
     * @param flags 格式化选项, 详见 {@link DateUtils#FORMAT_SHOW_TIME} 等
     * @return 格式化日期时间
     */
    protected static String formatDateTime(long time, int flags) {
        return DateUtils.formatDateTime(LshContextUtils.get(), time, flags);
    }
    ...
    protected static String formatDateTime(long time, int flags, Locale locale) {
        return DateUtils.formatDateRange(LshContextUtils.get(), new Formatter(new StringBuilder(50), locale), time, time, flags).toString();
    }
    ...
    public static String getWeekDayStringCN(long time) {
        return formatDateTime(time, DateUtils.FORMAT_SHOW_WEEKDAY, Locale.CHINA);
    }
    ...
    public static String getWeekDayString(long time, Locale locale) {
        return formatDateTime(time, DateUtils.FORMAT_SHOW_WEEKDAY, locale);
    }
